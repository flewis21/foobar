      ...
      // if (parType === "unknown" || !payDataD) {
        ...
        if (bpayType === "url" || bpayType === "text") {
          ...
          if ((bHtml && typeof bHtml?.getContent === "function") || (bhtmlPayL && typeof bhtmlPayL?.getContent === "function") || (bHtmlPL && typeof bHtmlPL?.getContent === "function") || (inHtml && typeof inHtml?.getContent() === "function")) {
            ...
            if (typeof bHtml?.getContent === "function") {
              ...
              if (typeof bhtmlPayL?.getContent === "function") {
                ...
                if (typeof bHtmlPL?.getContent === "function") {
                  ...
                  if (typeof inHtml?.getContent() === "function") {
                    console.log("event; inHtml " + inHtml?.getContent(), String(inHtml).length);
                    console.log("returning this from Renderer:html\n", JSON.stringify(dataOR));
                    return inHtml.evaluate()
                  }
                }
              }
            }
            ...
            console.log("returning this from Renderer:", JSON.stringify(dataOR));
...
function et(e) {
  ...
  if (funcCallParams.length === 0) {
    ...
    //     if (htmlArray.includes(firstArg)) {
    //       let foobarr0Index = htmlArray.findIndex(function (element) {
    //         return element === firstArg;
    //       });
    //       htmlDosArg = htmlArray[foobarr0Index];
    //     }
    //   } else if (htmlArray.includes(foobarr)) {
    //     let foobarrIndex = htmlArray.findIndex(function (element) {
    ...
    try {
      ...
      return renderFile(
        "Untitled2.html",
        {
          ...
          //   Object.values(this[libName].mis(rawFuncResult || data))[0],
          // aplot:
          //   payLoad.type === "text" || payLoad.type === "url"
          //     ? iframeSrc
          //     : JSON.stringify(payLoad),
      ...
      //   //                       }
      //   //                       else {
      //   //                         let newDataUrl = new URL(newHtmlContent.data);
      //   //                         if (newDataUrl) {
      //   //                           window.location.href = newDataUrl; // New type "url" for strings
      ...
      //                     const updatedClientE = {
      //                       parameter: {
      //                         func: "driveManager" //currentE.parameter.func, // Keep the original func
      //                         args: parsedE                 // Use the new parsed args
      //                       }
      //                     };
      //                     // --- MODIFICATION ENDS HERE ---

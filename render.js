...
var renderFile = function (file, argsObject, title) {
  ...
  try {
    if (file) {
      ...
      // END IF
      // Route[file] = argsObject
      // var research = foo.geneFrame(seoSheet(coUtility()[0].rndTitle).url
    ...
    <!doctype html>
      <html lang="en">
        ...
        <body id="renderFile" clas="flex-div static-fix" background-image="<?!= logo.getContent() ?>">
          ...
          <script>document.getElementById('func').addEventListener('change', <?!= renderArgsObject.funcClicked.getContent() ?>)</script>
          <script>document.getElementById('args').addEventListener('change', <?!= renderArgsObject.argsClicked.getCOntent() ?>)</script>
